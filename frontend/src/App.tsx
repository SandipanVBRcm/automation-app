...
import { citedSources } from './lib/sources'
...
export default function App() {
  ...
  async function deleteWorkspace() {
    ...
    const warning = 'Delete workspace ' + selected.name + '? This permanently deletes its conversations, skills, knowledge, and workspace files. Files saved directly in Downloads will remain. This cannot be undone.'
  ...
  async function send(override?: string) {
    ...
    try {
      ...
      const explicitDomain = ['off', 'auto', 'all'].includes(knowledgeMode) ? null : knowledgeMode
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <a className="brand" href="#" onClick={e => e.preventDefault()}><span className="brand-mark"><Orbit size={25} /></span> orbit<span className="local-tag">DEV</span></a>
      ...
      <div className="sidebar-bottom"><div className="local-note"><ShieldCheck size={17}/><div><strong>Your browser. Your workspace.</strong><small>History stored on this computer</small></div></div><div className="sidebar-controls"><button className="settings-button" onClick={() => setSettingsOpen(true)}><Settings2 size={16}/> Settings <span className={`dot ${health ? 'green' : ''}`}/></button><button className="theme-toggle" title={dark ? 'Switch to light mode' : 'Switch to dark mode'} onClick={() => setDark(!dark)}>{dark ? <Sun size={14}/> : <Moon size={14}/>}</button></div></div>
    </aside>
    ...
    {settingsOpen && <SettingsModal health={health} onSaved={values => setHealth(current => current ? { ...current, ...values } : current)} onClose={() => setSettingsOpen(false)} />}
  </div>
}
