...
Use the workspace selector below the Orbit logo to create or switch workspaces. Workspaces keep conversations, skills, internal files, and browser sessions separate. When File MCP is enabled, user-facing files and completed browser downloads save directly in the PC Downloads folder for every workspace. Deleting a workspace does not delete files saved directly in Downloads.
