...
from app.runtime_settings import RuntimeValues, runtime_settings
...
async def health():
    ...
    return {'status': 'ok', 'browser_service': 'available' if shutil.which('node') else 'unavailable', 'mcp': 'available' if installed else 'not_installed', 'providers': {p: configured(p) for p in ['openai', 'azure', 'anthropic']}, 'models': {'openai': settings.openai_model, 'azure': settings.azure_openai_deployment, 'anthropic': settings.anthropic_model}, **runtime_settings.values.model_dump()}


@app.patch('/api/settings')
async def update_settings(body: RuntimeValues):
    try:
        previous = runtime_settings.values.playwright_mcp_enabled
        updated = runtime_settings.update(body)
    except OSError as exc:
        raise HTTPException(503, 'Unable to save settings on this computer.') from exc
    if previous and not updated.playwright_mcp_enabled:
        await asyncio.gather(*(
            session.close() for session in list(manager.sessions.values())
            if session.mcp.status in ('connected', 'starting')
        ))
    return updated
