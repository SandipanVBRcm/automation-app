...
QUERY_STOP_WORDS = set('a an the of for to from in on at and or is are was were be has have with about please give show tell me us what which who how find get provide information details detail demographic demographics record records patient customer'.split())
...
class RagService:
    ...
    async def reindex(self, workspace_id: str, document_id: str):
        document = await self.get_document(workspace_id, document_id)
        if document['status'] in {'UPLOADED', 'PARSING', 'CHUNKING', 'EMBEDDING', 'INDEXING'}:
    ...
    async def delete_document(self, workspace_id: str, document_id: str):
        document = await self.get_document(workspace_id, document_id)
        # Stop indexing before cleanup so it cannot recreate deleted chunks.
        active = [task for task in self.tasks if not task.done() and task.get_name() == f'rag:{workspace_id}:{document_id}']
        for task in active:
            task.cancel()
        if active:
            await asyncio.gather(*active, return_exceptions=True)
    ...
    async def query(self, workspace_id: str, question: str, domain_id: str | None = None, *, queries: list[str] | None = None, document_id: str | None = None, content_type: str | None = None, top_k: int | None = None) -> RetrievalResult:
        ...
        if not question.strip():
            raise ValueError('Enter a non-empty knowledge query.')
        if document_id:
            document = await self.get_document(workspace_id, document_id)
            if domain_id and document['domain_id'] != domain_id:
                raise ValueError('Knowledge document not found in the selected domain.')
        ...
        for variant, vector in zip(variants, embedded, strict=True):
            ...
            terms = list(dict.fromkeys(term for term in WORD.findall(variant.lower()) if term not in QUERY_STOP_WORDS))[:12]
            if terms and hasattr(self.store, 'lexical_search'):
                rows += await self.store.lexical_search(workspace_id, terms, settings.rag_candidate_k, filters)
            for row in rows:
                distance = float(row.get('distance', 1))
                vector_score = max(0.0, 1.0 - distance)
                lexical = self._lexical_score(variant, row.get('text', ''))
                exact = bool(terms) and set(terms).issubset(set(WORD.findall(row.get('text', '').lower())))
                score = max(0.85 * vector_score + 0.15 * lexical, 1.0 if exact else 0.0)
