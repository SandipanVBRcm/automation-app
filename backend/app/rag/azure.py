...
class CosmosVectorStore:
    ...
    async def lexical_search(self, workspace_id, terms, limit, filters):
        """Find exact record terms even when vectors rank similar records higher."""
        if not terms:
            return []
        where = ["c.workspace_id = @workspace_id", "c.kind = 'chunk'"]
        ...
        for name in ('domain_id', 'document_id', 'content_type'):
            if filters.get(name):
                where.append(f'c.{name} = @{name}')
                parameters.append({'name': f'@{name}', 'value': filters[name]})
        for index, term in enumerate(terms):
            where.append(f'CONTAINS(c.text, @term{index}, true)')
            parameters.append({'name': f'@term{index}', 'value': term})
        query = (f'SELECT TOP {int(limit)} c.id, c.document_id, c.domain_id, c.chunk_index, c.text, '
                 'c.page_number, c.section, c.source_file, c.content_type '
