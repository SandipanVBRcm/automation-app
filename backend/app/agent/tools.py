...
RAG_SEARCH = {
    ...
    'description': (
        ...
        'uploaded documents, named patient/customer records, or other workspace knowledge. Search before asking for identifying details when a knowledge domain is selected. Do not use it for trivial arithmetic, generic knowledge, or live web facts. '
...
class ToolRegistry:
    ...
    def definitions(self):
        ...
        if not runtime_settings.values.file_mcp_enabled:
            blocked.add('browser_file_upload')
        browser = []
        if runtime_settings.values.playwright_mcp_enabled:
        ...
        knowledge = [{'name': 'knowledge_' + name, 'description': desc, 'parameters': schema.model_json_schema()} for name, (schema, desc) in KNOWLEDGE_TOOLS.items()] if runtime_settings.values.file_mcp_enabled else []
        rag = [] if getattr(self.session, 'use_knowledge', None) is False else [RAG_SEARCH]
        files = tool_definitions() if runtime_settings.values.file_mcp_enabled else []
        if files and not runtime_settings.values.playwright_mcp_enabled:
            files = [tool for tool in files if tool['name'] != 'file_save_screenshot']
        return browser + files + skills + knowledge + rag + [PARALLEL_AGENT]
    ...
    async def execute(self, name, args):
        ...
        if category == 'file' and not runtime_settings.values.file_mcp_enabled:
            raise RuntimeError('File MCP is disabled in Settings.')
        if name.startswith('knowledge_') and not runtime_settings.values.file_mcp_enabled:
            raise RuntimeError('File MCP is disabled in Settings.')
        ...
        if category == 'knowledge':
            if name == 'rag_search':
                from app.rag.service import rag_service, tool_payload, grounding_prompt
                parsed = RagSearchArgs.model_validate(args)
                if self.session.use_knowledge is False:
                    raise ValueError('Workspace knowledge is disabled for this request.')
                try:
                    ...
                    self.session.rag_context = ('WORKSPACE KNOWLEDGE: Retrieval failed. Explain that the knowledge service could not be reached or the selected domain is unavailable. Ask the user to check Knowledge infrastructure health. Do not invent record details or describe this as an empty search result.')
                    self.session.rag_sources = []
                    self.session.rag_debug = None
                    raise RuntimeError('Knowledge retrieval failed. Check Knowledge infrastructure health and the selected domain.') from exc
                self.session.rag_context = grounding_prompt(result)
