...
def test_workspace_storage_delete_is_scoped_to_selected_workspace(tmp_path, monkeypatch):
    ...
    selected_downloads = legacy.policy.root / 'Orbit Workspaces' / selected_id
    retained_downloads = legacy.policy.root / 'Orbit Workspaces' / retained_id
    selected_downloads.mkdir(parents=True)
    retained_downloads.mkdir(parents=True)
    selected_downloads.joinpath('report.txt').write_text('delete')
    retained_downloads.joinpath('report.txt').write_text('keep')
    asyncio.run(workspaces.downloads_for(selected_id).execute(
        'create_file', {'path': 'direct.txt', 'content': 'keep in Downloads'},
    ))
    ...
    assert (legacy.policy.root / 'direct.txt').read_text() == 'keep in Downloads'


def test_workspace_download_services_save_directly_to_downloads(tmp_path, monkeypatch):
    legacy = FileService(tmp_path / 'Downloads', blocked_names={'Orbit Workspaces'})
    monkeypatch.setattr(workspaces, 'downloads', legacy)
    workspaces.downloads_for.cache_clear()
    ...
    assert (legacy.policy.root / 'same.txt').read_text() == 'first'
    assert not (legacy.policy.root / 'Orbit Workspaces').exists()
