...
class FakeVectorStore:
    ...
    async def lexical_search(self, workspace_id, terms, limit, filters):
        rows = [dict(item) for (partition, _), item in self.values.items()
                if partition == workspace_id and item.get('kind') == 'chunk'
                and not any(value and item.get(name) != value for name, value in filters.items())
                and all(term.lower() in item.get('text', '').lower() for term in terms)]
        return rows[:limit]
...
@pytest.mark.asyncio
async def test_exact_name_candidate_outside_vector_results():
    class HybridStore(FakeVectorStore):
        async def vector_search(self, workspace_id, embedding, limit, filters):
            return [{'id': 'wrong', 'document_id': 'other', 'text': 'Unrelated demographics', 'distance': .1}]
        async def lexical_search(self, workspace_id, terms, limit, filters):
            assert terms == ['morgan', 'test']
            return [{'id': 'match', 'document_id': 'record', 'source_file': 'synthetic.csv', 'text': 'Name: Morgan Test | City: Example City'}]
    service = RagService(HybridStore(), FakeBlobStore(), FakeEmbeddings())
    result = await service.query(DEFAULT_WORKSPACE_ID, 'give the Morgan Test demographic information')
    assert result.chunks[0]['id'] == 'match'
    assert result.sources[0]['file_name'] == 'synthetic.csv'


@pytest.mark.asyncio
async def test_delete_cancels_inflight_indexing():
    started = asyncio.Event()
    class SlowEmbedding(FakeEmbeddings):
        async def embed(self, texts):
            started.set()
            await asyncio.Event().wait()
    vectors, blobs = FakeVectorStore(), FakeBlobStore()
    service = RagService(vectors, blobs, SlowEmbedding())
    domain = await service.create_domain(DEFAULT_WORKSPACE_ID, 'Delete active')
    document = await service.upload(DEFAULT_WORKSPACE_ID, domain['id'], 'active.txt', 'text/plain', b'Synthetic finance record')
    await asyncio.wait_for(started.wait(), 5)
    await service.delete_document(DEFAULT_WORKSPACE_ID, document['id'])
    assert await vectors.get(document['id'], DEFAULT_WORKSPACE_ID) is None
    assert not blobs.values
    assert not any(item.get('document_id') == document['id'] for item in vectors.values.values())


@pytest.mark.asyncio
async def test_queued_reindex_is_rejected_and_preview_download_work():
    service = RagService(FakeVectorStore(), FakeBlobStore(), FakeEmbeddings())
    domain = await service.create_domain(DEFAULT_WORKSPACE_ID, 'Preview')
    document = await service.upload(DEFAULT_WORKSPACE_ID, domain['id'], 'preview.txt', 'text/plain', b'finance reimbursement test')
    with pytest.raises(ValueError, match='already processing'):
        await service.reindex(DEFAULT_WORKSPACE_ID, document['id'])
    await next(iter(service.tasks))
    result = await service.query(DEFAULT_WORKSPACE_ID, 'finance reimbursement', domain['id'])
    preview = await service.chunk_preview(DEFAULT_WORKSPACE_ID, result.sources[0]['chunk_id'])
    assert 'reimbursement' in preview['text']
    _, data = await service.download(DEFAULT_WORKSPACE_ID, document['id'])
    assert data == b'finance reimbursement test'


@pytest.mark.asyncio
async def test_lexical_query_keeps_scope_and_parameterizes_terms(monkeypatch):
    from app.rag.azure import CosmosVectorStore
    store = CosmosVectorStore()
    async def query(sql, parameters, workspace):
        params = {p['name']: p['value'] for p in parameters}
        assert workspace == DEFAULT_WORKSPACE_ID
        assert "c.workspace_id = @workspace_id" in sql
        assert "c.domain_id = @domain_id" in sql
        assert "c.document_id = @document_id" in sql
        assert params['@domain_id'] == 'domain'
        assert params['@document_id'] == 'document'
        assert params['@term0'] == "o'brien"
        assert "o'brien" not in sql
        return []
    monkeypatch.setattr(store, 'query_items', query)
    await store.lexical_search(DEFAULT_WORKSPACE_ID, ["o'brien"], 10, {'domain_id':'domain', 'document_id':'document'})
