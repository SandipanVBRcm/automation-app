...
@pytest.mark.asyncio
@pytest.mark.parametrize('message', ['open google', 'Please open Google', 'go to google.com', 'click the first link', 'search cats on Google', 'hello'])
async def test_browser_or_greeting_never_searches_selected_domain(setup_run, monkeypatch, message):
    from app.rag.service import rag_service
    session, db, _ = setup_run
    async def forbidden(*args, **kwargs):
        raise AssertionError('Non-knowledge task must not retrieve')
    choices = iter([
        Decision('', [{'id': 'a', 'name': 'browser_initialize', 'arguments': {}}]),
        Decision('', [{'id': 'b', 'name': 'browser_navigate', 'arguments': {'url':'https://www.google.com'}}]),
        Decision('Google is open.', []),
    ])
    async def model(provider, system, messages, definitions, emit):
        assert 'rag_search' not in {tool['name'] for tool in definitions}
        return next(choices)
    monkeypatch.setattr(rag_service, 'query', forbidden)
    monkeypatch.setattr(runner, 'decide', model)
    await runner.run(session, message, 'azure', True, 'selected-domain')
    response = db.conversation(session.session_id)['messages'][-1]
    assert response['content'] == 'Google is open.'
    assert not response.get('sources')
    assert not response.get('retrieval')
    assert 'rag_search' not in {row['tool_name'] for row in db.rows('SELECT tool_name FROM tool_calls')}
